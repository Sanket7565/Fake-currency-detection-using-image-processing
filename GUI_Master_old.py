...
#
lbl = tk.Label(root, text="Fake Currency Detection System", font=('times', 35,' bold '), height=1, width=56,bg="brown",fg="white")
lbl.place(x=0, y=0)
...
logo_lb1 = tk.Label(root, image=logo_image)
logo_lb1.image = logo_image
logo_lb1.place(x=40, y=2)


frame_alpr = tk.LabelFrame(root, text=" --Process-- ", width=220, height=400, bd=5, font=('times', 14, ' bold '),bg="sky blue")
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='D:/All Code/Project Done/Fake_Currency 100% Code', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
